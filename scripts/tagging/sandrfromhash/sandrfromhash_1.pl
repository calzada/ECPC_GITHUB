...
while ( @ARGV ) {
    ...
    my $number_of_characters = length($original);
    my $original_sin_ext = substr($original,0,$number_of_characters - 4);
    #EN LINUX SERÍA ASÍ system("mv",$input_file,$original_sin_ext . $old_suffix);
    #EN LINUX SERÍA ASÍ system("mv",$output_file,$original);   
    
    rename($input_file,$original_sin_ext . $old_suffix);
    rename($output_file,$original);
