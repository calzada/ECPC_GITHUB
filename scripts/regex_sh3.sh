...
perl -pi -e 's/(<h4|H4)(.*?center.*?>THE.*?PARLIAMENTARY.*?DEBATES.*?OFFICIAL REPORT.*?UNITED.*?KINGDOM.*?OF.*?GREAT.*?BRITAIN.*?AND.*?NORTHERN.*?IRELAND.*?SERIES.*?VOLUME.*?\p{N}+?.*?VOLUME OF.*?SESSION\p{Z}*?\p{N}+?.\p{N}+?.*?)(<\/h4>|<\/H4>)(.*?)(XZXZ.xml version..1\.0..*?YWYWXZXZPYWYWXZXZbodyYWYWXZXZPYWYW)/$5XZXZPYWYWXZXZomit desc="title"YWYW$1$2$3XZXZ\/omitYWYWXZXZPYWYWXZXZ/g' *.html
perl -pi -e 's/.*?(XZXZ.xml version..1\.0.)/$1/g' *.html

perl -pi -e 's/(XZXZomit desc="title"YWYW).*?(THE).*?(PARLIAMENTARY).*?(DEBATES).*?(OFFICIAL).*?(REPORT).*?(IN).*?(THE)/$1$2 $3 $4 $5 $6 $7 $8/g' *.html

perl -pi -e 's/(SESSION).*?(OF).*?(.*?PARLIAMENT).*?(OF).*?(THE).*?(UNITED.*?KINGDOM.*?OF.*?GREAT.*?BRITAIN.*?AND.*NORTHERN.*?IRELAND).*?\[(WHICH.*?OPENED.*?)\].*?(YEAR.*?OF.*?THE)/$1 $2 $3 $4 $5 $6 ($7) $8/g' *.html

perl -pi -e 's/(REIGN).*?(OF).*?(HER).*?(MAJESTY).*?(QUEEN).*?(ELIZABETH.*?II).*?(\p{Lu}+.*?SERIES).*?(VOLUME.*?\p{N}+).*?(\p{Lu}.*?VOLUME.*?OF.*?SESSION.*?\p{N}+-\p{N}+)<+?/$1 $2 $3 $4 $5 $6 $7 $8 $9XZXZ\/omitYWYWXZXZPYWYWXZXZ</g' *.html

perl -pi -e 's/\p{Z}+(H4|h4)>//g' *.html
...
perl -pi -e 's/<(H4|h4).*?center.*?(The.*?House.*?met.*?at.*?)<\/(H4|h4)>.*?<(H4|h4|H3|h3).*?center.*?(PRAYERS|Prayers).*?<\/(H4|h4|H3|h3)>.*?<(H4|h4).*?center.*?\[(.*?in.*?the.*?Chair.*?)\].*?<\/(H4|h4)>/XZXZomit desc="starttime"YWYW$2XZXZ\/omitYWYWXZXZPYWYWXZXZomit desc="title"YWYW$5XZXZ\/omitYWYWXZXZPYWYWXZXZomit desc="chair"YWYW$8XZXZ\/omitYWYW/g' *.html

#BORRAR FECHAS NO DESEADAS
perl -pi -e 's/<(b|B)>\p{N}+\p{Z}+\p{L}+\p{Z}+\p{N}+\p{Z}+:\p{Z}+\p{L}+\p{Z}+\p{N}+<\/(b|B)>//g;' *.html


#ETIQUETAR "... WAS ASKED"
perl -pi -e 's/<(H4|h4).*?>(.*?was asked.*?)(<.*?>)*?<\/(H4|h4)>/XZXZomit desc="addressee"YZYZ$2XZXZ\/omitYWYW/g' *.html

#ETIQUETAR HORA ENTRE OMITS: <H5>
perl -pi -e 's/<(H5|h5).*?>(\p{N}+\.*?\p{N}*?.*?)(am|pm)<\/(H5|h5)>/XZXZomit desc="time"YWYW$2$3XZXZ\/omitYWYW/g;' *.html

	
#ETIQUETAR HEADINGS DE DIVERSOS TIPOS
perl -pi -e 's/<(H|h)\p{N}+.*?>(.*?)<\/(H|h)\p{N}+>/XZXZheading number="XY"YWYW$2XZXZ\/headingYWYW/g' *.html
...
perl -pi -e 's/<!-- division -->/XZXZomit desc="div"YWYW/g;' *.html

#CIERRE DE LA DIVISION. ATENCIÓN. AQUÍ EL REEMPLAZO SE INICIA CON UN ESPACIO EN BLANCO
perl -pi -e 's/(<i>\p{Z}*?)*?(Question accordingly.*?)(<\!-- end division -->|<\/i><br>)/ $2XZXZ\/omitYWYW/g;' *.html

#ETIQUETAR MÁS OMITS

#PARA ETIQUETAR NUMERACIÓN REFERIDA A DOCUMENTOS
perl -pi -e 's/\[(P)*?(\p{N}+)\p{P}*?\]/XZXZomit desc="doc"YWYW$1$2XZXZ\/omitYWYW/g;' *.html
perl -pi -e 's/\[\p{Z}*?Bill\p{Z}*?(\p{N}+)\p{P}*?\]/XZXZomit desc="doc"YWYWBill $1$2XZXZ\/omitYWYW/g;' *.html

#PARA CAMBIAR CORCHETES POR PARÉNTESIS EN TÍTULOS..
perl -pi -e 's/(\p{Z}*?)\[(\p{Z}*?\p{Lu}{3,})+\](\p{Z}*?)/$1($2)$3/g;' *.html

#PARA CAMBIAR CORCHETES POR PARÉNTESIS DESPUÉS DE "THAT THIS HOUSE DO NOW ADJOURN
perl -pi -e 's/now adjourn.*?<i>\p{Z}*?.*?\[(.*?)\](<\/i>)?/now adjourn\. \($1\)/g' *.html

#PARA CAMBIAR CORCHETES POR PARÉNTESIS EN THIS DAY.
perl -pi -e 's/\[(\p{Z}*?this day\p{Z}*?\p{P}*?\p{Z}*?)\]/\($1\)/g;' *.html
...
perl -pi -e 's/\[\p{Z}*?(<i>)*?\p{Z}*?(Mr\.*?|Mrs\.*?|Ms|Dr\.*?|Sir|Rev.*?|.on\.*?\p{Z}*?\.ember|Lady|Lord)*?(\p{Z}*?\p{L}+?\p{Z}*?\p{L}*?\.*?\p{Z}+?\p{L}+?)(-\p{L}+?)*?(\p{Z}*?\.*?)(\p{Z}*?<\/i>\p{Z}*?)*?(\.*?)\p{Z}*?\]/XZXZomit desc="refname"YWYW\$1$2$3$4$5$6$7XZXZ\/omitYWYW/g' *.html

perl -pi -e 's/\[(.*?)\]/XZXZomit desc="refdoc"YWYW$1XZXZ\/omitYWYW/g' *.html
...
perl -pi -e 's/(-->)(<i>)*?\p{Z}*?(rose—)/$1XZXZomit desc="reaction"YWYW$3XZXZ\/omitYWYW/g' *.html
perl -pi -e 's/(YWYW)\p{Z}+?(rose—)/$1$2/g' *.html

#QUITAR NÚMEROS QUE ANTECEDEN A LAS PREGUNTAS ESCRITAS. ATENCIÓN POR SI HUBIERA UN Q.1 (QUE NO HE QUITADO)

perl -pi -e 's/(<b>)\p{Z}*?\p{N}+\./$1/g' *.html
perl -pi -e 's/(CONTENT="\p{N}+"-->\p{Z}*?)T\.*?\p{N}+\./$1/g' *.html
perl -pi -e 's/(<!--\p{Z}*?)\p{N}+/$1/g' *.html
perl -pi -e 's/(XZXZ\/headingYWYW\p{Z}*?)\p{N}+\./$1/g' *.html
perl -pi -e 's/(<\/a>)\p{Z}*?\p{N}+\./$1/g' *.html
...
perl -pi -e 's/<!--META NAME="Colno" CONTENT="\p{N}+"-->\p{Z}*?<A NAME="\p{N}+-\p{N}+_spnew\p{N}+">\p{Z}*?<\/A>\p{Z}*?<B>\p{Z}*?(\p{L}.*?on\.*?\p{Z}*?Members)\p{Z}*?(:)*?\p{Z}*?<\/B>\p{Z}*?<!--.*?-->\p{Z}*?(:)*?\p{Z}*?(\p{P}\p{L}|<|\p{L})/XZXZintervention id="inXYXY"YWYWXZXZPYWYWXZXZspeakerYWYWXZXZPYWYWXZXZnameYWYW$1XZXZ\/nameYWYWXZXZPYWYWXZXZ\/speakerYWYWXZXZPYWYWXZXZspeech id="XYXY" language="EN"YWYW$3/g' *.html

#SPNEW WITH DETAILS
perl -pi -e 's/<!--META NAME="Colno" CONTENT="\p{N}+"-->\p{Z}*?<A NAME="\p{N}+-\p{N}+_spnew\p{N}+">\p{Z}*?<\/A>\p{Z}*?<B>\p{Z}*?(\p{L}.*?\p{L})\p{Z}*?(\(.*?\))*?\p{Z}*?(:)*?\p{Z}*?<\/B>\p{Z}*?<!--(.*?)-->\p{Z}*?(\(.*?\))*?\p{Z}*?(:)*?\p{Z}*?(\p{L}|<)/XZXZintervention id="inXYXY"YWYWXZXZPYWYWXZXZspeakerYWYWXZXZPYWYWXZXZnameYWYW$4XZXZ\/nameYWYWXZXZPYWYWXZXZ\/speakerYWYWXZXZPYWYWXZXZspeech id="XYXY" language="EN"YWYW$7/g' *.html

#SPNEW WITHOUT DETAILS
perl -pi -e 's/<!--META NAME="Colno" CONTENT="\p{N}+"-->\p{Z}*?<A NAME="\p{N}+-\p{N}+_spnew\p{N}+">\p{Z}*?<\/A>\p{Z}*?<B>\p{Z}*?(\p{L}.*?\p{L})\p{Z}*?(:)*?\p{Z}*?<\/B>\p{Z}*?<!--(.*?)-->\p{Z}*?(:)*?\p{Z}*?(\p{L}|<|&#\p{N}+)/XZXZintervention id="inXYXY"YWYWXZXZPYWYWXZXZspeakerYWYWXZXZPYWYWXZXZnameYWYW$3XZXZ\/nameYWYWXZXZPYWYWXZXZ\/speakerYWYWXZXZPYWYWXZXZspeech id="XYXY" language="EN"YWYW$5/g' *.html

#ULTIMOS FLECOS

#PARA BORRAR DIVISION NO. 21 9: 29 PM. 
perl -pi -e 's/(Division No\.\p{Z}+?\p{N}+?XZXZPYWYW\p{N}+?:\p{N}+?\p{Z}+?)(am|pm)//g' *.html

#PARA ELMINAR LOS NÚMEROS (3.) ANTES DEL HEADING AL PRINCIPIO DE LÍNEA
perl -pi -e 's/(XZXZheading number=")XY(" type="sbhd\p{N}+"YWYW.*?XZXZ\/headingYWYWXZXZPYWYW)(\p{N}+)\.\p{Z}+(XZXZ)/$1$3$2$4/g' *.html

#PARA PONER HEADINGS EN LÍNEA APARTE
#perl -pi -e 's/(XZXZheading)/XZXZPYWYWXZXZPYWYW$1/g' *.html

#PARA ASEGURARNOS QUE NO HAY MÁS DE DOS \N DELANTE DE LOS HEADINGS
#perl -pi -e 's/XZXZPYWYWXZXZPYWYW(XZXZPYWYW)+(XZXZheading)/XZXZPYWYWXZXZPYWYW$2/g' *.html
...
perl -pi -e 's/(language="EN"YWYW.*?XZXZPYWYW)<A NAME = "\p{N}+?-\p{N}+?_para0">/$1/g' *.html
...
perl -pi -e 's/(XZXZomitYWYW.*?XZXZ\/omitYWYW)(XZXZPYWYWXZXZPYWYWXZXZPYWYWXZXZintervention)/$1XZXZ\/speechYWYWXZXZ\/interventionYWYW$2/g' *.html
...
perl -pi -e 's/(YWYW)(XZXZPYWYW)+?(XZXZintervention)/$1XZXZ\/speechYWYWXZXZPYWYWXZXZ\/interventionYWYWXZXZPYWYWXZXZPYWYW$3/g' *.html


#AÑADIR \N DESPUÉS DE LAS ETIQUETAS DE BODY Y /HEADING
perl -pi -e 's/

perl -pi -e 's/(XZXZ)(body|\/heading)(YWYW)XZXZPYWYW(XZXZ)/$1$2$3XZXZPYWYWXZXZPYWYW$4/g' *.html

#QUITAR ESPACIOS ANTES DE /SPEECH (POR SI ACASO)

#perl -pi -e 's/XZXZPYWYW(XZXZPYWYW)+(XZXZ\/speechYWYW)/XZXZPYWYWXZXZ\/speechYWYW/g' *.html

#CAMBIAR ORDEN DE HEADING /SPEECH E /INTERVENTION PARA QUE EL HEADING QUEDE FUERA DE LA INTERVENCIÓN

perl -pi -e 's/(XZXZheading number="XY" type="sbhd\p{N}+"YWYW.*?XZXZ\/headingYWYW)(XZXZPYWYW)*?(XZXZ\/speechYWYW(XZXZPYWYW)*?XZXZ\/interventionYWYW)/$3XZXZPYWYW$1/g' *.html

#ELIMINAR <.*?>
perl -pi -e 's/<.*?>//g' *.html

#QUITAR 3 ESPACIOS DE SALTOS DE PÁRRAFO INNECESARIOS
perl -pi -e 's/XZXZPYWYWXZXZPYWYWXZXZPYWYWXZXZPYWYW/XZXZPYWYW/g' *.html
perl -pi -e 's/XZXZPYWYWXZXZPYWYWXZXZPYWYW/XZXZPYWYW/g' *.html
perl -pi -e 's/XZXZPYWYW(XZXZPYWYW)+(XZXZ\/speechYWYW)/XZXZPYWYWXZXZ\/speechYWYW/g' *.html
...
#SUSTITUIR XZXZ POR <
perl -pi -e 's/XZXZ/</g' *.html

#SUSTITUIR YWYW POR >
perl -pi -e 's/YWYW/>/g' *.html


#BORRAR HORAS
perl -pi -e 's/^\p{N}+\.\p{N}+\p{Z}+(am|pm)//g' *.html
	

#DEJAR NOMBRE Y APELLIDO (QUITÁR TÍTULO)
perl -pi -e 's/(<name>).*?(Mr\.*?|Mrs|Dr\.*?|Miss|Sir|Hon|Rt Hon|Rev.*?)\p{Z}+(.*?)(<\/name>)/$1$3$4/g' *.html

#BORRAR ROSE
perl -pi -e 's/<i>\p{Z}*?rose&#151;//g' *.html
